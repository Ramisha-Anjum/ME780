function createAllCurvedLanePlots_dt0005_v2
...
%   AFS_vs_TV-main/src/createAllCurvedLanePlots_dt0005_v2.m
%
% Run with:
%   createAllCurvedLanePlots_dt0005_v2
...
% Create the metrics table robustly, even when a selected SDI run has a
% shorter time range or contains nonfinite samples.
...
rowTemplate = struct( ...
    'Controller', "", ...
    'EvaluationStart_s', evaluationStart, ...
    'EvaluationEnd_s', evaluationEnd, ...
    'CrossTrackRMSE_m', NaN, ...
    'CrossTrackPeak_m', NaN, ...
    'HeadingRMSE_deg', NaN, ...
    'HeadingPeak_deg', NaN, ...
    'YawRateRMSE_rad_s', NaN, ...
    'YawRatePeak_rad_s', NaN, ...
    'BetaPeak_deg', NaN, ...
    'AyPeak_m_s2', NaN, ...
    'SteeringPeak_deg', NaN, ...
    'MzPeak_Nm', NaN);

metricRows = repmat(rowTemplate, nRuns, 1);

for k = 1:nRuns
    t = double(runData(k).time(:));
    ...
    if ~any(mask)
        error('createAllCurvedLanePlots_dt0005:EmptyMetricInterval', ...
            ['Run "%s" has no samples between %.1f and %.1f s. ' ...
             'Its available time range is %.6g to %.6g s. ' ...
             'Delete any short test run from SDI or rename the complete ' ...
             '10-second run so it contains Baseline, AFS, or TV.'], ...
            char(runData(k).name), evaluationStart, evaluationEnd, ...
            min(t), max(t));
    end

    ey = double(runData(k).crossTrackError(mask));
    epsi = double(runData(k).headingError(mask));
    yawError = double(runData(k).rRef(mask) - runData(k).r(mask));
    betaValues = double(runData(k).beta(mask));
    ayValues = double(runData(k).ay(mask));
    steeringValues = double(runData(k).deltaF(mask));
    mzValues = double(runData(k).Mz(mask));

    ey = ey(isfinite(ey));
    epsi = epsi(isfinite(epsi));
    yawError = yawError(isfinite(yawError));
    betaValues = betaValues(isfinite(betaValues));
    ayValues = ayValues(isfinite(ayValues));
    steeringValues = steeringValues(isfinite(steeringValues));
    mzValues = mzValues(isfinite(mzValues));

    metricRows(k).Controller = string(runData(k).name);
    metricRows(k).CrossTrackRMSE_m = safeRMS(ey);
    metricRows(k).CrossTrackPeak_m = safePeak(ey);
    metricRows(k).HeadingRMSE_deg = rad2deg(safeRMS(epsi));
    metricRows(k).HeadingPeak_deg = rad2deg(safePeak(epsi));
    metricRows(k).YawRateRMSE_rad_s = safeRMS(yawError);
    metricRows(k).YawRatePeak_rad_s = safePeak(yawError);
    metricRows(k).BetaPeak_deg = rad2deg(safePeak(betaValues));
    metricRows(k).AyPeak_m_s2 = safePeak(ayValues);
    metricRows(k).SteeringPeak_deg = ...
        rad2deg(safePeak(steeringValues));
    metricRows(k).MzPeak_Nm = safePeak(mzValues);
end

% struct2table avoids the table-constructor row-size ambiguity that caused
% the previous error.
metrics = struct2table(metricRows);
end

function value = safeRMS(values)
if isempty(values)
    value = NaN;
else
...
function value = safePeak(values)
if isempty(values)
    value = NaN;
else
